...
const STATE_TONE: Record<ConnectorCard["state"], string> = {
  ...
  configured: "bg-sky-500/10 text-sky-600 dark:text-sky-300",
...
function StateIcon({ state }: { state: ConnectorCard["state"] }) {
  ...
  return <ShieldCheck className="h-5 w-5 shrink-0 text-sky-500" />;
}
...
function formatCheckedAt(value: string | null) {
  ...
  return `Dernière vérification : ${new Date(value).toLocaleString("fr-FR", {
    day: "2-digit",
    month: "2-digit",
    hour: "2-digit",
    minute: "2-digit",
...
export function ConnectionsPanel() {
  ...
  return (
    <div className="space-y-4">
      <AdminCard
        title="Connexions"
        description="Chaque service listé alimente une fonction réelle d’Angel OS. L’état affiché provient d’un appel réel, pas d’une supposition."
      >
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-background p-3">
          <p className="text-sm text-muted-foreground">
            {loading
              ...
              : `${connectedCount} service${connectedCount > 1 ? "s" : ""} vérifié${connectedCount > 1 ? "s" : ""} sur ${cards.length}.`}
          </p>
          <button
            type="button"
        ...
        {error ? (
          <div className="mt-3 flex gap-2 rounded-xl border border-red-500/25 bg-red-500/10 px-3 py-2.5 text-xs text-red-700 dark:text-red-100">
                  ...
                  <section key={card.key} className="rounded-2xl border border-border bg-background p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${STATE_TONE[card.state]}`}>
                    ...
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      {card.features.map((feature) => (
                        <span
                          key={feature}
                          className="rounded-lg border border-border bg-muted/50 px-2 py-1 text-[11px] text-muted-foreground"
                        >
                          {feature}
                        </span>
                      ))}
                    ...
                    <dl className="mt-3 space-y-1 text-[11px] leading-5 text-muted-foreground">
                      <div className="flex gap-1.5">
                        <dt className="font-medium text-foreground">Accès :</dt>
                        <dd>{card.via}</dd>
                      </div>
                      <div className="flex gap-1.5">
                        <dt className="font-medium text-foreground">Permissions :</dt>
                        <dd>{card.permissions.join(" · ")}</dd>
                      </div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button
                        ...
                        className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-border bg-background px-3 text-sm text-foreground disabled:opacity-40"
                      >
                        {busy === card.key ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}
                        Tester
                      </button>
                      {link && card.state === "connected" ? (
                        <a
                          href={link.href}
                          className="inline-flex min-h-10 items-center rounded-xl bg-foreground px-3.5 text-sm font-semibold text-background"
                        >
                          {link.label}
                        </a>
                      ) : null}
                    ...
                    {card.needsManualSetup ? (
                      <p className="mt-3 rounded-xl border border-border bg-muted/30 px-3 py-2 text-[11px] leading-5 text-muted-foreground">
                        Ce service demande une configuration hors Angel OS (clé ou connecteur à relier). Tant qu’elle
                        manque, aucune fonction ne dépend de lui : rien n’est cassé, la carte reste simplement inactive.
                      </p>
                    ) : null}
                  </section>
                );
      ...
      <AdminCard
        title="Accès de l’IA"
        description="Angel OS IA n’utilise que les services effectivement vérifiés ci-dessus."
      >
        <div className="flex items-start gap-3">
          <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-red-500" />
          <p className="text-xs leading-5 text-muted-foreground">
            Les identifiants Google passent par la passerelle de connecteurs : aucun jeton n’est stocké dans le projet
            ni exposé au navigateur. Le renouvellement des accès est géré côté passerelle.
          </p>
        </div>
      </AdminCard>
    </div>

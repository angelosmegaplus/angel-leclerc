...
function sourceLabel(source?: MovixOfficialSource["source"]) {
  if (source === "persisted") return "Dernière source saine";
      ...
      <div className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#08090b] shadow-[0_30px_100px_rgba(0,0,0,.42)]">
        <div className="flex flex-col gap-4 border-b border-white/10 bg-[radial-gradient(circle_at_12%_0%,rgba(220,38,38,.16),transparent_30%)] p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-xs leading-5 text-white/35">La maintenance inscrit automatiquement la dernière source valide. L’ouverture du bac à sable reste volontaire : un clic suffit pour l’activer.</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-400/15 bg-emerald-400/[.06] px-3 py-1.5 font-mono text-[10px] uppercase tracking-[.12em] text-emerald-200/65"><ShieldCheck className="h-3 w-3" />sandbox prêt</span>
        ...
        <div id="cinema-sandbox-frame" className="scroll-mt-3 p-3 sm:p-4">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3 px-1">
            <div><p className="font-mono text-[10px] uppercase tracking-[.16em] text-white/25">Source inscrite</p><p className="mt-1 font-mono text-xs text-white/45">{hostOf(primary.url)}</p></div>
            <div className="flex flex-wrap gap-2">
              {!embed ? <button type="button" onClick={() => openIntegrated(primary.url)} className="rounded-xl bg-red-600 px-4 py-2.5 text-xs font-semibold text-white hover:bg-red-500">Ouvrir et activer le bac à sable</button> : null}

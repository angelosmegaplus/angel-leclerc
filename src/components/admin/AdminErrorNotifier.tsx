...
function compactRaw(value: unknown): string {
  ...
  return (isHtmlPayload(raw) ? stripHtml(raw) : raw).replace(/\s+/g, " ").trim();
}
...
function cleanReason(value: unknown): string {
  ...
  if (/500|502|503|504|internal server|service unavailable|bad gateway|gateway timeout/i.test(compact)) return "Une opération serveur n’a pas terminé correctement.";
...
function summarizeProblem(value: unknown): string {
  ...
  if (/failed to fetch|networkerror|network request failed/i.test(compact)) return "La requête n’a pas pu atteindre sa destination. L’interface reste disponible et le contrôle pourra être retenté.";
  ...
  if (/429|too many requests|rate limit/i.test(compact)) return "Une limite de débit a été atteinte. Angel OS pourra retenter plus tard.";
  if (/500|502|503|504|internal server|service unavailable|bad gateway|gateway timeout/i.test(compact)) return "Une opération serveur a échoué. Cette erreur ne doit plus faire croire que toute l’administration est indisponible.";
...
function technicalDetail(value: unknown): string {
  ...
  if (isHtmlPayload(raw)) return "Réponse HTML serveur masquée. Ce type de réponse de fond n’est plus affiché comme alerte globale.";
...
function playAlertTone() {
  ...
  } catch { /* l’alerte visuelle suffit */ }
}
    ...
    <div className="pointer-events-none fixed inset-x-0 top-[calc(.75rem+env(safe-area-inset-top))] z-[100] flex justify-center px-3 sm:top-[calc(1rem+env(safe-area-inset-top))]">
      <div key={notice.id} role="alert" aria-live="assertive" className="pointer-events-auto w-full max-w-md animate-in slide-in-from-top-4 fade-in duration-300 overflow-hidden rounded-[1.35rem] border border-red-500/35 bg-[#0b0d10]/96 shadow-[0_24px_80px_rgba(0,0,0,.55)] backdrop-blur-xl">
        <div className="flex items-start gap-3 p-3.5 sm:p-4">
          <div className="relative shrink-0"><img src="/angel-os/logo.png" alt="Angel OS" className="h-11 w-11 rounded-xl border border-red-500/25 object-cover" /><span className="absolute -bottom-1 -right-1 grid h-5 w-5 place-items-center rounded-full border border-red-400/30 bg-red-500 text-white"><AlertTriangle className="h-3 w-3" /></span></div>
          ...
          <button type="button" onClick={() => setNotice(null)} aria-label="Fermer l’alerte" className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-white/10 bg-white/[.04] text-white/55"><X className="h-4 w-4" /></button>
        </div>

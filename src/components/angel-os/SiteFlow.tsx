...
export function SiteFlow() {
  return (
    <section id="site" className="relative overflow-x-clip border-y border-white/10 bg-[#050607] px-5 py-20 sm:px-8 lg:px-12">
      ...
      <div className="relative mx-auto max-w-6xl">
        <div className="flex flex-col gap-6 border-b border-white/10 pb-8 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h2 className="mt-4 max-w-4xl break-words font-display text-3xl font-bold tracking-[-.04em] sm:text-5xl">Comprendre le noyau, puis ouvrir le capot.</h2>
            <p className="mt-4 max-w-3xl break-words text-base leading-relaxed text-white/55">Chaque étape possède deux zones volontairement séparées : une lecture immédiate pour comprendre le rôle, puis une console technique pour voir le fonctionnement réel.</p>
          </div>
        ...
        <div className="mt-10 space-y-6">
          {steps.map((step) => (
            <article key={step.eyebrow} className="group relative overflow-x-clip rounded-[2rem] border border-white/10 bg-[#090b0d]/95 shadow-[0_24px_80px_rgba(0,0,0,.35)] transition duration-500 hover:-translate-y-1 hover:border-red-500/30">
              ...
              <div className="grid lg:grid-cols-[.95fr_1.05fr]">
                <div className="relative min-w-0 p-6 sm:p-8 lg:p-10">
                  ...
                  <h3 className="mt-5 break-words font-display text-2xl font-bold tracking-[-.03em] text-white sm:text-4xl">{step.title}</h3>
                  <p className="mt-4 max-w-xl break-words text-base leading-relaxed text-white/72 sm:text-lg">{step.simple}</p>
                ...
                <div className="relative min-w-0 border-t border-white/10 bg-black/45 p-6 sm:p-8 lg:border-l lg:border-t-0 lg:p-10">
                  ...
                  <div className="relative min-w-0">
                    ...
                    <p className="mt-5 max-w-3xl break-words font-mono text-[12px] leading-7 text-white/43">{step.technical}</p>
                    ...
                    <div className="mt-6 rounded-xl border border-white/10 bg-[#020303]">
                      ...
                      <div className="relative overflow-x-auto px-4 py-4 font-mono text-[11px] leading-6 text-red-300/65">
                        <div className="absolute inset-y-0 left-0 w-px animate-pulse bg-red-400/50" />
                        <div className="min-w-max">$ {step.code}<span className="ml-1 inline-block h-3 w-[2px] animate-pulse bg-red-400/80" /></div>
                        <div className="mt-2 min-w-max text-white/20">status: ok · route: core/{step.eyebrow} · latency: internal</div>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div className="mt-10 overflow-x-clip rounded-[1.75rem] border border-white/10 bg-[#08090b]">
          <div className="flex items-center gap-3 border-b border-white/10 px-5 py-4"><span className="h-2 w-2 animate-pulse rounded-full bg-red-400"/><p className="break-words font-mono text-[10px] uppercase tracking-[.2em] text-white/45">Implémentations utilisant Angel OS</p></div>
          <div className="grid gap-px bg-white/10 sm:grid-cols-2">
            <div className="min-w-0 bg-[#08090b] p-5"><p className="text-sm font-semibold text-white/75">Angel OS IA</p><p className="mt-2 break-words text-sm leading-relaxed text-white/40">Distribution spécialisée qui ajoute l’intelligence artificielle, les tâches planifiées et les automatisations au-dessus du noyau.</p></div>
            <div className="min-w-0 bg-[#08090b] p-5"><p className="text-sm font-semibold text-white/75">angel-leclerc.fr</p><p className="mt-2 break-words text-sm leading-relaxed text-white/40">Application web utilisant certaines capacités d’Angel OS IA. Ce n’est pas le noyau lui-même.</p></div>
          </div>
        </div>
      </div>
    </section>

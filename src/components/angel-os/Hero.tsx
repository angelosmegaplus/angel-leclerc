      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid items-center gap-12 pt-14 lg:grid-cols-[1.1fr_.9fr] lg:pt-20">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-xl font-medium text-white/85 sm:text-2xl">Une architecture numérique modulaire conçue pour piloter, centraliser et automatiser un environnement web complet.</p>
            <p className="mt-5 max-w-2xl text-base leading-relaxed text-white/55 sm:text-lg">Conçu et piloté par Angel Leclerc, Angel OS associe ChatGPT, automatisations planifiées, composants applicatifs, services connectés et outils de déploiement dans une même chaîne opérationnelle. L’intelligence artificielle intervient comme couche de raisonnement et de coordination, tandis que le système assure l’exécution, le suivi et la continuité des opérations.</p>
            <p className="mt-4 max-w-2xl text-sm leading-relaxed text-white/40 sm:text-base">Le projet illustre une utilisation avancée de l’IA appliquée à un environnement réel : veille, administration, maintenance, contenus, candidatures, données, contrôles techniques, publication et automatisation de tâches récurrentes.</p>
          ...
          <div className="rounded-[2rem] border border-white/10 bg-black/80 p-6 shadow-2xl transition duration-300 hover:-translate-y-1 hover:border-red-500/25">
            <div className="flex items-center gap-5"><img src="/angel-os/logo.png" alt="Logo Angel OS" className="h-24 w-24 rounded-2xl object-cover"/><div><p className="text-xs uppercase tracking-[.22em] text-white/35">Architecture actuelle</p><p className="mt-2 text-2xl font-bold">Angel OS IA</p><p className="mt-1 text-sm text-white/45">orchestration assistée par IA</p></div></div>
            <div className="mt-6 space-y-3 font-mono text-xs text-white/60"><p><span className="text-red-400">reasoning</span> → ChatGPT / IA</p><p><span className="text-red-400">core</span> → Angel OS</p><p><span className="text-red-400">automation</span> → tâches planifiées</p><p><span className="text-red-400">services</span> → web / données / connecteurs</p><p><span className="text-red-400">delivery</span> → GitHub / Vercel</p></div>
          </div>
        </div>
      </div>
    </section>

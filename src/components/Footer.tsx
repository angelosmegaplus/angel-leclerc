import { Linkedin, Instagram, Facebook, Flame } from "lucide-react";
...
const navLinks = [
  ...
  { to: "/flamme/social", label: "Flamme social — Bêta" },
...
export function Footer() {
  return <footer className="w-full border-t border-border bg-card"><div className="container-tight py-16">
    <div className="grid gap-10 md:grid-cols-3">
      <div><p className="font-display text-lg font-bold tracking-tight text-foreground">Angel Leclerc <span className="text-primary">Communication</span></p><p className="mt-2 text-sm text-muted-foreground">Gestion de projet · Conseil · Rédaction</p><p className="mt-6 text-sm italic text-muted-foreground">Donner du souffle à vos idées.</p><Link to="/flamme/social" className="mt-5 inline-flex items-center gap-2 rounded-full border border-border bg-background px-3.5 py-2 text-xs font-semibold text-muted-foreground transition-colors hover:border-primary hover:text-foreground"><Flame size={15} className="text-primary" />Flamme social <span className="rounded-full bg-primary/10 px-1.5 py-0.5 text-[9px] font-bold uppercase text-primary">Bêta</span></Link></div>
    ...
    <div className="mt-12 border-t border-border pt-8 text-xs leading-relaxed text-muted-foreground">
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p>© {new Date().getFullYear()} Angel Leclerc Communication. Tous droits réservés.</p>
        <Link to="/angel-os" aria-label="Découvrir Angel OS" className="mt-3 inline-flex items-center gap-2 text-[11px] text-muted-foreground/80 transition-colors hover:text-foreground"><img src="/angel-os/logo.png" alt="" className="h-4 w-4 rounded object-contain" /><span className="tracking-wide">Angel OS × Lovable</span></Link>

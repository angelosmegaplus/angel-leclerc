  ...
  server: {
    handlers: {
      GET: async () => {
        ...
        // Local/dev environments do not expose Vercel's deployment SHA.
        if (!deployedSha) {
          return Response.json(
            {
              maintenance: false,
              reason: "deployment-sha-unavailable",
              deployedSha: null,
              latestSha: null,
            },
            { headers },
        ...
        try {
          const response = await fetch(GITHUB_BRANCH_API, {
            headers: {
          ...
          return Response.json(
            {
              maintenance: latestSha !== deployedSha,
              reason: latestSha !== deployedSha ? "deployment-in-progress" : "up-to-date",
              deployedSha,
              latestSha,
            },
            { headers },
          ...
          // Fail open: a temporary GitHub/API outage must not take the whole site offline.
          return Response.json(
            {
              maintenance: false,

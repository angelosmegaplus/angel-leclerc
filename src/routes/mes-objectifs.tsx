...
export const Route = createFileRoute("/mes-objectifs")({
  head: () => ({
    meta: [
      { title: "Pistes d'avenir — Angel Leclerc" },
      { name: "description", content: "Quelques pistes de formation et de métiers explorées dans la communication, le journalisme et la radio." },
...
function MesObjectifsPage() {
  return (
    <main className="overflow-hidden pb-24 md:pb-0">
      <AnimatedSection><section className="section-padding bg-background"><div className="container-tight"><SectionHeader eyebrow="Repères évolutifs" title="Pistes pour la suite" intro="Quelques pistes de formation et de métiers que j'explore actuellement. Elles peuvent évoluer avec les expériences, les rencontres et les opportunités : cette page ne constitue pas un plan de carrière figé." /><div className="mx-auto mt-6 flex max-w-2xl items-start gap-3 rounded-2xl border border-border bg-muted/30 px-4 py-3 text-sm text-muted-foreground"><TriangleAlert size={18} className="mt-0.5 shrink-0 text-primary" /><p><span className="font-semibold text-foreground">Des pistes, pas une feuille de route définitive.</span> Cette page sert surtout à conserver quelques directions possibles et les formations associées.</p></div></div></section></AnimatedSection>
      <AnimatedSection><section className="section-padding bg-background"><div className="container-tight"><SectionHeader eyebrow="Piste · Journalisme" title="Journalisme d'investigation" intro="Enquêter, confronter les versions, retrouver des documents et des témoins, vérifier les faits et construire un récit solide." /><div className="mx-auto mt-8 max-w-3xl overflow-hidden rounded-2xl border border-border bg-card shadow-sm md:mt-12"><YouTubeEmbed id="xcnYwQRZcGQ" title="Comment travaillent les journalistes d'investigation ?" /></div><div className="mt-6 grid gap-4 md:grid-cols-3"><Card><Search className="text-primary" size={22} /><h3 className="mt-4 font-semibold">Enquêter sur le terrain</h3></Card><Card><ShieldCheck className="text-primary" size={22} /><h3 className="mt-4 font-semibold">Vérifier les sources</h3></Card><Card><Waves className="text-primary" size={22} /><h3 className="mt-4 font-semibold">Construire l'enquête</h3></Card></div><p className="mt-10 text-center text-xs font-semibold uppercase tracking-widest text-primary">Écoles et formations envisageables</p><div className="mt-5 grid gap-4 md:grid-cols-2">{journalistSchools.map((school) => <SchoolCard key={school.name} school={school} />)}</div></div></section></AnimatedSection>
      <AnimatedSection><section className="section-padding bg-muted/40"><div className="container-tight"><SectionHeader eyebrow="Autre piste · Radio" title="Animation radio" intro="Une autre voie possible, davantage centrée sur le divertissement et la personnalité d'antenne : concevoir des émissions rythmées, faire vivre le direct, recevoir des invités et créer une vraie interaction avec les auditeurs." /><div className="mx-auto mt-8 max-w-3xl overflow-hidden rounded-2xl border border-border bg-card shadow-sm md:mt-12"><YouTubeEmbed id="2eC3bFEnTlA" title="Présentation du métier d'animateur radio" /></div><p className="mt-10 text-center text-xs font-semibold uppercase tracking-widest text-primary">Formations spécialisées</p><div className="mt-5 grid gap-4 md:grid-cols-3">{hostSchools.map((school) => <SchoolCard key={school.name} school={school} />)}</div></div></section></AnimatedSection>
    </main>
  );
}

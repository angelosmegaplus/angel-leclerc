...
function MesObjectifsPage() {
  return (
    <main className="overflow-hidden pb-24 md:pb-0">
      ...
      <AnimatedSection><section className="section-padding bg-muted/40"><div className="container-tight"><SectionHeader eyebrow="Plan B · Radio" title="Animateur radio" intro="Une deuxième voie possible, davantage centrée sur l'antenne : concevoir une émission, maîtriser le direct, mener des interviews et développer une identité d'antenne." /><div className="mx-auto mt-8 max-w-3xl overflow-hidden rounded-2xl border border-border bg-card shadow-sm md:mt-12"><YouTubeEmbed id="2eC3bFEnTlA" title="Présentation du métier d'animateur radio" /></div><p className="mt-10 text-center text-xs font-semibold uppercase tracking-widest text-primary">Formations spécialisées</p><div className="mt-5 grid gap-4 md:grid-cols-3">{hostSchools.map((school) => <SchoolCard key={school.name} school={school} />)}</div></div></section></AnimatedSection>
    </main>
  );
}

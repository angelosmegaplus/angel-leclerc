    ...
    <section className="relative border-b border-border bg-black text-white">
      ...
      <div className="container-tight relative py-20 md:py-28">
        <div className="flex items-center gap-4">
          <span className="flex h-16 w-16 items-center justify-center rounded-2xl border border-orange-400/30 bg-orange-500/10 shadow-2xl shadow-orange-500/20 md:h-20 md:w-20"><LockKeyhole className="h-8 w-8 text-orange-400 md:h-10 md:w-10" /></span>
          <div><p className="text-xs font-semibold uppercase tracking-[.22em] text-orange-400">Angel OS · Security layer</p><h1 className="font-display text-4xl font-bold tracking-tight md:text-6xl">Angel Guard OS</h1></div>
        </div>
        ...
        <div className="mt-8 flex flex-wrap gap-3"><Button asChild size="lg" className="bg-orange-500 text-white hover:bg-orange-600"><Link to="/angel-os">Voir Angel OS <ArrowRight className="ml-2 h-4 w-4" /></Link></Button><Button asChild size="lg" variant="outline" className="border-white/20 bg-white/5 text-white hover:bg-white/10"><Link to="/auth">Accès privé</Link></Button></div>
      </div>
    </section>

    <section className="container-tight py-16 md:py-20"><p className="text-xs font-semibold uppercase tracking-[.18em] text-orange-500">Boucle autonome</p><h2 className="mt-3 max-w-3xl font-display text-3xl font-bold md:text-4xl">Détecter → décider → agir → vérifier → apprendre</h2>
      <div className="mt-8 grid gap-3 md:grid-cols-5">{["Détection", "Décision", "Action", "Vérification", "Mémoire"].map((item, i) => <div key={item} className="relative rounded-3xl border border-border bg-card p-5"><span className="text-xs font-bold text-orange-500">0{i + 1}</span><p className="mt-4 font-semibold">{item}</p>{i < 4 ? <ArrowRight className="absolute -right-4 top-1/2 hidden h-4 w-4 text-muted-foreground md:block" /> : null}</div>)}</div>
    </section>

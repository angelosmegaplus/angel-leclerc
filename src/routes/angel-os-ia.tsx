    ...
    <main className="min-h-screen overflow-hidden bg-[#050607] text-white">
      ...
      <section className="border-y border-white/10 bg-white/[.02] px-5 py-16 sm:px-8 lg:px-12">
        <div className="mx-auto grid max-w-6xl gap-5 md:grid-cols-3">
          {[
            ["Angel OS", "Le noyau commun : événements, modules, configuration, capacités et adaptateurs."],
            ["Angel OS IA", "Ma distribution personnelle : IA, automatisations, administration et outils de pilotage."],
            ["angel-leclerc.fr", "Une application web qui utilise le Core sans lui confier son design ni ses fonctions essentielles."],
          ].map(([title, text]) => <article key={title} className="rounded-3xl border border-white/10 bg-[#0c0e11] p-7 transition duration-300 hover:-translate-y-1 hover:border-red-500/25"><h2 className="text-xl font-semibold">{title}</h2><p className="mt-3 text-sm leading-relaxed text-white/50">{text}</p></article>)}
        </div>
      </section>

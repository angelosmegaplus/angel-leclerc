...
import { ArrowLeft, Check, ExternalLink, Loader2, MessageSquare, Play, Save, Star } from "lucide-react";
...
const COMMENT_PREFIX = "angel-os-film-comment-v1:";
...
function FilmDetailPage() {
  ...
  useEffect(() => {
    try {
      const value = localStorage.getItem(`${COMMENT_PREFIX}${id}`) ?? "";
      setComment(value);
      setSavedComment(value);
    } catch { setComment(""); setSavedComment(""); }
  }, [id]);
  ...
  const saveComment = () => {
    const clean = comment.trim().slice(0, 4000);
    try {
      if (clean) localStorage.setItem(`${COMMENT_PREFIX}${id}`, clean);
      else localStorage.removeItem(`${COMMENT_PREFIX}${id}`);
    } catch { /* optional */ }
    setComment(clean);
    setSavedComment(clean);
  };
    ...
    <section className="relative overflow-hidden border-b border-white/10">
      ...
      <div className="relative mx-auto max-w-[1320px] px-4 pb-8 pt-5 sm:px-8 lg:px-10 lg:pb-12">
        ...
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="mt-8 grid gap-6 sm:grid-cols-[180px_1fr] lg:grid-cols-[245px_1fr] lg:items-end">
          ...
          <div><div className="flex flex-wrap items-center gap-2 text-xs text-white/55"><span>{candidate.mediaType === "movie" ? "Film" : "Série"}</span><span>•</span><span>{year}</span>{runtime ? <><span>•</span><span>{runtime}</span></> : null}</div><h1 className="mt-3 text-4xl font-semibold tracking-[-.055em] sm:text-5xl lg:text-7xl">{detail?.title || candidate.title}</h1>{detail?.tagline ? <p className="mt-2 max-w-2xl text-sm italic text-white/45 sm:text-base">{detail.tagline}</p> : null}<div className="mt-4 flex flex-wrap items-center gap-2">{rating ? <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1.5 text-sm"><Star className="h-4 w-4 fill-yellow-400 text-yellow-400" /> {rating.toFixed(1)}</span> : null}{genres.map((genre) => <span key={genre} className="rounded-full border border-white/10 bg-black/25 px-3 py-1.5 text-xs text-white/65">{genre}</span>)}</div><p className="mt-5 max-w-3xl text-base leading-7 text-white/75 sm:text-lg">{synopsis}</p>{detail?.trailerKey ? <button type="button" onClick={() => setPlayingTrailer((value) => !value)} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-semibold text-black"><Play className="h-4 w-4 fill-current" />{playingTrailer ? "Masquer la bande-annonce" : "Lire la bande-annonce"}</button> : null}</div>
        </motion.div>
      </div>
    </section>
    ...
    <div className="mx-auto max-w-[1320px] px-4 py-8 sm:px-8 lg:px-10">
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
        <div className="space-y-10">
          <section className="rounded-2xl border border-white/10 bg-white/[.025] p-5 sm:p-6"><h2 className="text-xl font-semibold">Synopsis complet</h2><p className="mt-4 whitespace-pre-line text-sm leading-7 text-white/70 sm:text-base">{synopsis}</p></section>
          ...
          <section className="rounded-2xl border border-white/10 bg-white/[.025] p-5"><div className="flex items-center gap-2"><MessageSquare className="h-5 w-5 text-red-300" /><h2 className="text-xl font-semibold">Tes notes et commentaires</h2></div><p className="mt-2 text-xs leading-5 text-white/40">Espace privé pour garder ton avis, des remarques ou ce que tu veux retenir du film.</p><textarea value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Écris ton commentaire personnel…" rows={6} className="mt-4 w-full resize-y rounded-xl border border-white/10 bg-black/30 p-3 text-sm leading-6 text-white outline-none placeholder:text-white/25 focus:border-red-400/40" /><div className="mt-3 flex items-center justify-between gap-3"><span className="text-[11px] text-white/30">{comment.length}/4000</span><button type="button" onClick={saveComment} className="inline-flex items-center gap-2 rounded-xl bg-red-500 px-4 py-2.5 text-xs font-semibold text-white hover:bg-red-400"><Save className="h-4 w-4" />{savedComment === comment.trim() ? "Enregistré" : "Enregistrer"}</button></div></section>

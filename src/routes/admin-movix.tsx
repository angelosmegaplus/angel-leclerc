...
function FilmsSeriesPage() {
  ...
  return <main className="min-h-screen bg-[#050607] px-4 py-6 text-white sm:px-7 lg:px-10">
    <div className="mx-auto max-w-[1300px]">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div><div className="flex items-center gap-2 text-red-300"><Film className="h-5 w-5" /><span className="font-mono text-xs uppercase tracking-[.18em]">Angel OS · recommandations</span></div><h1 className="mt-2 text-4xl font-semibold tracking-[-.05em] sm:text-5xl">Films et séries</h1><p className="mt-2 max-w-2xl text-sm text-white/50">Ta petite sélection du moment. Ouvre une affiche pour voir sa fiche complète.</p></div>
      ...
      <section className="mt-9">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {ranked.map(({ candidate, score }, index) => <article key={candidate.id} className="overflow-hidden rounded-[1.5rem] border border-white/10 bg-[#0c0d0f]">
            <a href={`/admin-movix/${candidate.id}`} className="group block">
              <div className="relative aspect-[2/3] overflow-hidden bg-[#101114]">
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black via-black/75 to-transparent px-5 pb-5 pt-24">
                  <div className="mb-3 flex flex-wrap items-center gap-2"><span className="rounded-full bg-red-600 px-2.5 py-1 text-[10px] font-bold uppercase text-white">{index === 0 ? "Choix du jour" : candidate.mediaType === "movie" ? "Film" : "Série"}</span><span className="rounded-full bg-black/80 px-2.5 py-1 text-[10px] text-white">{Math.round(score * 100)}%</span></div>
                  <h3 className="text-2xl font-semibold tracking-tight text-white">{candidate.title}</h3><p className="mt-1 text-sm text-white/65">{candidate.year} · {candidate.genreLabel}</p>
                </div>
              </div>
            </a>
            <div className="space-y-2.5 border-t border-white/5 p-4">
              <p className="pb-1 text-sm text-white/50">Tu connais déjà ce titre ?</p>
              <button type="button" onClick={() => registerSignal(candidate, { completion: 0.5, liked: true, rejected: false })} className="flex min-h-12 w-full items-center gap-3 rounded-xl border border-white/10 bg-white/[.04] px-4 text-left text-base text-white/80"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-md border border-white/20"><Check className="h-3.5 w-3.5" /></span>J’ai aimé</button>
              <button type="button" onClick={() => registerSignal(candidate, { completion: 0.5, liked: false, rejected: true })} className="flex min-h-12 w-full items-center gap-3 rounded-xl border border-white/10 bg-white/[.04] px-4 text-left text-base text-white/80"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-md border border-white/20"><Check className="h-3.5 w-3.5" /></span>Je n’ai pas aimé</button>
              <button type="button" onClick={() => registerSignal(candidate, { completion: 1, rejected: false })} className="flex min-h-12 w-full items-center gap-3 rounded-xl border border-white/10 bg-white/[.04] px-4 text-left text-base text-white/80"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-md border border-white/20"><Check className="h-3.5 w-3.5" /></span>Je l’ai déjà vu</button>
            </div>
          </article>)}
        </div>
      </section>

...
async function sendIndependentFallback(
  data: ConversationalContactInput,
  projectType: string,
  description: string,
): Promise<void> {
  ...
  if (!response.ok) {
    console.error("[contact-chat] independent fallback failed", response.status);
    throw new Error("La demande n'a pas pu être transmise. Merci de réessayer dans quelques instants.");
  }

  const result = (await response.json().catch(() => null)) as { success?: boolean } | null;
  if (result && result.success === false) {
    console.error("[contact-chat] independent fallback rejected", result);
    throw new Error("La demande n'a pas pu être transmise. Merci de réessayer dans quelques instants.");
  }
}
...
export async function processConversationalContact(data: ConversationalContactInput) {
    ...
    // Chemin historique : conserve exactement le stockage et les e-mails Lovable
    // lorsqu'ils sont configurés sur l'hébergement.
    if (hasSupabaseAdmin) {
      ...
      // Supabase est disponible mais pas le fournisseur mail Lovable : on conserve
      // l'enregistrement puis on transmet la notification par le backend indépendant.
      await sendIndependentFallback(data, projectType, description);
      return { ok: true as const };
    }

    // Vercel autonome : même validation, même récapitulatif, même adresse destinataire,

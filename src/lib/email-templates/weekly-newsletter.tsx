  ...
  >
    ...
    {articles.map((article, index) => (
      <Section key={article.url}>
        {index > 0 && (
          <Hr style={{ border: "none", borderTop: `1px solid ${line}`, margin: "24px 0" }} />
        )}
        {article.imageUrl && (
          <Img
        ...
        <Text
          style={{ ...textStyle, fontSize: 18, fontWeight: 700, color: ink, margin: "0 0 8px 0" }}
        >
          {article.title}
        </Text>
        {article.excerpt && (
          <Text style={{ ...textStyle, color: "#4b4b4b", margin: "0 0 14px 0" }}>
            {article.excerpt}
          </Text>
        )}

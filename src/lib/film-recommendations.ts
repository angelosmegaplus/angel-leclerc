...
function maxValue(values: number[]) { return Math.max(...values, 1); }
export function scoreCandidate(candidate: RecommendationCandidate, profile: TasteProfile, nowYear = new Date().getFullYear()) {
  ...
  const genreScore = candidate.genreIds.length ? Math.min(candidate.genreIds.reduce((sum, id) => sum + ((profile.genres[id] || 0) / maxGenre), 0) / candidate.genreIds.length, 1) : 0;
  const maxKeyword = maxValue(Object.values(profile.keywords));
  const keywordScore = candidate.keywords.length ? Math.min(candidate.keywords.reduce((sum, keyword) => sum + ((profile.keywords[keyword] || 0) / maxKeyword), 0) / candidate.keywords.length, 1) : 0;
  ...
  const peopleScore = relevantPeople.length ? Math.min(relevantPeople.reduce((sum, person) => sum + ((profile.people[person] || 0) / maxPerson), 0) / Math.min(relevantPeople.length, 3), 1) : 0;
  ...
  return (genreScore * 0.35) + (keywordScore * 0.25) + (peopleScore * 0.15) + (popularityNorm * 0.10) + (recencyScore * 0.10) + (formatMatch * 0.05);
}
...
export function selectDailyRecommendations(candidates: RecommendationCandidate[], signals: ViewingSignal[]) {
  ...
  const ranked = candidates.filter((candidate) => !excluded.has(candidate.id)).map((candidate) => {
  ...
  for (const item of ranked) {
    ...
    if (selected.length < 3 && primaryGenre && seenPrimaryGenres.has(primaryGenre)) continue;
    selected.push(item);
    if (primaryGenre) seenPrimaryGenres.add(primaryGenre);
  }

...
function stableDailyJitter(id: string) {
  let h = daySeed();
...
export function selectDailyRecommendations(candidates: RecommendationCandidate[], signals: ViewingSignal[]) {
  ...
  const ranked = candidates
    ...
    .map((candidate) => {
      ...
      const freshness = stableDailyJitter(candidate.id) * 0.025;
      return { candidate, score: Math.min(1, base + freshness) };
    })
    .sort((a, b) => b.score - a.score);

  // Trois propositions maximum, avec une légère diversité de genres.
  const selected: typeof ranked = [];
  const seenPrimaryGenres = new Set<number>();
  for (const item of ranked) {
    if (selected.length >= 3) break;
    const primaryGenre = item.candidate.genreIds[0];
    if (selected.length < 2 && primaryGenre && seenPrimaryGenres.has(primaryGenre)) continue;
    selected.push(item);

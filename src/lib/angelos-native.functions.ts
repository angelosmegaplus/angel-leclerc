...
type Row = { id: string; created_at?: string; updated_at?: string; [key: string]: JsonValue | undefined };
...
export const listAngelRows = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data: input }) => {
    ...
    return [...rows].sort((a, b) => String(b[input.orderBy] ?? "").localeCompare(String(a[input.orderBy] ?? ""))).slice(0, 500) as any;
  });
...
export const saveAngelRow = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data: input }) => {
    ...
    const row = { ...(previous ?? {}), ...(input.values as Record<string, JsonValue>), id, created_at: previous?.created_at ?? now, updated_at: now } as Row;
    await data.set(namespace(input.table), id, row);
    await data.set("activity.native", crypto.randomUUID(), { source: "user", action: input.id ? "update" : "create", entityType: input.table, entityId: id, at: now });
    return row as any;
  });
...
export const removeAngelRow = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data: input }) => {
    ...
    await data.set("activity.native", crypto.randomUUID(), { source: "user", action: "delete", entityType: input.table, entityId: input.id, at: new Date().toISOString() });
    return { ok: true };
  });

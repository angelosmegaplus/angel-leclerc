...
const state = { tabs: [], activeId: null, bookmarks: [], history: [], settings: { home: HOME, blockTrackers: true, httpsFirst: true, guardian: true } };
...
async function refreshPrivacyStats(){
  try{
    const s=await window.flamingbox.getPrivacyStats();
    $('statTrackers').textContent=s.blockedTrackers||0;
    $('statHttps').textContent=s.httpsUpgrades||0;
    $('statPopups').textContent=s.popupsBlocked||0;
    $('statGuardian').textContent=s.guardianAlerts||0;
    $('statDownloads').textContent=s.downloadsObserved||0;
    $('statRiskyDownloads').textContent=s.riskyDownloadsBlocked||0;
  }catch{}
}
...
function showGuardianAlert(payload){
  const box=$('guardianStatus');
  box.textContent=payload?.message||'FlamingBox Guardian a détecté une activité inhabituelle.';
...
$('bookmark').onclick=toggleBookmark;$('library').onclick=()=>{panel.classList.toggle('hidden');refreshPrivacyStats();};$('closePanel').onclick=()=>panel.classList.add('hidden');$('importData').onclick=doImport;
$('passwords').onclick=()=>createTab('https://passwords.google.com/');
$('security').onclick=()=>{$('securityPanel').classList.toggle('hidden');$('securityTitle').textContent='FlamingBox Shield';$('securityText').textContent='GPC et DNT actifs, HTTPS prioritaire, stockage tiers partitionné, trackers limités, Guardian réseau, téléchargements risqués filtrés et permissions sensibles temporaires.';};
...
$('guardian').onchange=async(e)=>{state.settings=await window.flamingbox.updateSettings({guardian:e.target.checked});};
window.flamingbox.onOpenTab((url,meta)=>createTab(url,{private:false,payment:!!meta.payment}));
window.flamingbox.onGuardianAlert(showGuardianAlert);

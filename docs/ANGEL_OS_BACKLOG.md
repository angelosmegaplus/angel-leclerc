...
- [ ] Corriger et vérifier de bout en bout Angel AI dans tout l’espace administrateur : les vraies questions doivent appeler le moteur IA principal, conserver le contexte et afficher une réponse conversationnelle ; le moteur local ne doit intervenir qu’en secours réel. Contrôler les logs, erreurs OpenAI, timeouts, quotas, anciens messages locaux parasites, routage des commandes et réponses persistées. Tester sur l’accueil admin, la recherche universelle et tout autre point d’entrée IA avant de considérer ce point terminé.
  ...
  - Renforcement du 16 août 2026 : le chemin commun n’avait plus de vrai secours Vercel AI Gateway. Le commit `f6d95168f7543d165fb6474557aa656f16cdf694` conserve OpenAI direct en premier choix et ajoute un fallback officiel `AI_GATEWAY_API_KEY` via `https://ai-gateway.vercel.sh/v1/responses`, avec cooldown, timeout, parsing et supervision séparés. Aucun secret n’est journalisé.
  - Validation production du 16 août 2026 sur la release exacte `f6d95168f7543d165fb6474557aa656f16cdf694` : CI complet vert (secret hygiene, lint, build, type-check, output), `/api/angel-os/health` `healthy:true`, OpenAI `configured:true/reachable:true`, trois requêtes réelles `POST /api/assistant` réussies en HTTP 200 (`site`, `contact`, conversation multi-tour avec mémoire ORION) avec texte exploitable et `reason=ok`, et contribution IA de la veille confirmée à `5/68`. La mémoire secondaire Supabase peut toujours signaler une table `ai_actions` absente sans faire tomber le cœur IA.
  - Le fallback Vercel AI Gateway est désormais réellement implémenté mais n’a pas été forcé en production : OpenAI direct est sain et il serait incorrect de désactiver/modifier un secret sain uniquement pour provoquer un échec. Il doit être exercé naturellement seulement si `AI_GATEWAY_API_KEY` est configurée et qu’OpenAI direct devient indisponible.
